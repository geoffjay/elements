...
dnl very similar to ELEM_AUTO_RESULT, but does not finalize the
dnl detection; it assumes that more checks will follow.
AC_DEFUN([ELEM_AUTO_PRE], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$var` != xno" && eval "test x`echo '$'$found` = xno"; then
                ELEM_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([ELEM_AUTO_RESULT], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$found` = xyes"; then
                ELEM_AUTO_ENABLED([$name], [$feature])
	else
                ELEM_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([ELEM_AUTO_PKG], [
